...
# Taken and adapted from Luke Ditria's GitHub account:
...
class ModelTrainer(nn.Module):
    ...
    def load_checkpoint(self):
        ...
        if os.path.isfile(self.save_path):
            # Load Checkpoint
            check_point = torch.load(self.save_path, weights_only=True)
    ...
    def run_training(self, n_epochs):
        train_accuracy = 0.
        val_accuracy = 0.
        progress_bar = trange(self.start_epoch, n_epochs, leave=False, desc='Epoch')
        for epoch in progress_bar:
            ...
            train_accuracy = self.evaluate_model(train_test_val='train')
            val_accuracy = self.evaluate_model(train_test_val='val')

            # Save model if current validation accuracy is greater than the previous best
            if val_accuracy > self.best_valid_acc:
    ...
    def evaluate_model(self, train_test_val="test"):
        ...
        self.eval()
        with torch.no_grad():
            for _, (x, y) in enumerate(tqdm(loader, leave=False, desc=state)):
