...
class CUB200(Dataset):
    def __init__(self, dataset_path, image_size, transform, train_test=1, return_masks=False):
        super().__init__()

        # Dataset found here
        # https://www.kaggle.com/datasets/wenewone/cub2002011
        
        class_list_path = os.path.join(dataset_path, "CUB_200_2011/image_class_labels.txt")
        self.data_df = pd.read_csv(class_list_path, sep=" ", names=["index", "class"])

        data_list_path = os.path.join(dataset_path, "CUB_200_2011/images.txt")
        cub200_df = pd.read_csv(data_list_path, sep=" ", names=["index", "file_path"])

        bbox_list_path = os.path.join(dataset_path, "CUB_200_2011/bounding_boxes.txt")
        bbox_df = pd.read_csv(bbox_list_path, sep=" ", names=["index", "x", "y", "width", "height"])

        # Use custom test/train split 1/0
        test_train_split_path = os.path.join(dataset_path, "test_train_split.txt")
        split_df = pd.read_csv(test_train_split_path, sep=" ", names=["index", "split"])
        ...
        self.data_df = self.data_df[self.data_df.split != train_test]  # train_test = 0 for training, 1 for testing

        self.return_masks = return_masks
        self.image_size = image_size
        self.transform = transform

        self.data_set_root = dataset_path
